-- Two additions to event registration:
--
-- 1. Per-field opt-in uniqueness for CUSTOM fields (Name/Email/Phone
--    already have their own dedicated columns + partial unique indexes
--    from migration 0069 — this is a separate, generic mechanism for
--    whatever else an organizer adds, driven by a `unique: true` flag the
--    field editor now lets them set — src/lib/events/registration-
--    validation.ts). Checked against the live jsonb answers of other
--    non-cancelled registrations for the same event, since there's no
--    fixed column (or index) to check against for an arbitrary
--    organizer-defined field. This is a soft, read-then-insert check —
--    same tradeoff as the confirmation_code retry loop already has, and
--    acceptable here since it's a best-effort convenience for optional
--    custom fields, not the core email/phone identity guarantee those
--    partial unique indexes already enforce.
--
-- 2. Registration now also closes 1 hour before the event's own start
--    time, regardless of registration_closes_at — you can already be
--    signed up, but nobody should be able to register for something
--    that's about to start or already has.
...
declare
  ...
  taken_count int;
begin
  select id, organization_id, registration_fields, registration_enabled, registration_capacity, registration_closes_at, start_at
    into target_event
    from public.events
  ...
  if now() > target_event.start_at - interval '1 hour' then
    raise exception 'Registration for this event has closed.';
  end if;
